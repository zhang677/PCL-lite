...
from reinforce.actions import query_affine_py_body_once
import os
import json
import csv

def parallel_impl(model_name, base_path, example_path, task_path, exp_name, num_samples, temperature=0.7):
    temp_base_dir = os.path.join(base_path, exp_name)
    if not os.path.exists(temp_base_dir):
        os.makedirs(temp_base_dir)
    config = Storage()
    storage = Storage()
    key = "query_affine_py_body"
    query_affine_py_body_once(key, {
...
if __name__ == "__main__":
    ...
    for row in rows:
        exp_type = row['Type']
        task_path = row['Task'].replace('.yaml', '.py')
        exp_name = exp_type + "/" + task_path.split('/')[-1].split('.')[0].replace('test', f'{args.num_samples}')
        temp_dir, stats = parallel_impl(args.model_name, args.base_path, args.example_path, task_path, exp_name, args.num_samples, args.temperature)
