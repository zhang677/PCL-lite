...
class Transpose(step.Fn):
    def __init__(self, input, output):
        super().__init__("Transpose", input, output)
    
    def apply(self, input):
        return [input[0].T]
        
    
fn_transpose_0 = Transpose(step.Buffer(step.Scalar("float"), [E, M]), step.Buffer(step.Scalar("float"), [M, E]))
...
fn_transpose_1 = Transpose(step.Buffer(step.Multihot(step.Scalar("float"), M), [E]), step.Buffer(step.Multihot(step.Scalar("float"), E), [M]))
...
fn_top3 = Top3(step.Buffer(step.Scalar("float"), [M]), step.Multihot(step.Scalar("float"), M))
